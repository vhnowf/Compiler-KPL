...
Token* readIdentKeyword(void) {
  ...
  while(currentChar != - 1 && (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT)){
        if(colnum == MAX_IDENT_LEN){
          error(ERR_IDENTTOOLONG, lineNo, colNo);
          return token;
        }
        token->string[colnum] = (char) currentChar;
        colnum ++;
        readChar();
      }
  token->string[colnum] = '\0';
...
Token* getToken(void) {
  ...
  case CHAR_PERIOD:  
    ...
    readChar(); 
    switch (charCodes[currentChar]) {
          case CHAR_RPAR: token = makeToken(SB_RSEL, lineNo, colNo); readChar(); return token;
  ...
  case CHAR_COLON: 
    ...
    readChar(); 
    switch (charCodes[currentChar]) {
          case CHAR_EQ: token = makeToken(SB_ASSIGN, lineNo, colNo);readChar();return token;
          default: return token;
        }
  case CHAR_SEMICOLON: 
  ...
  case CHAR_LPAR: 
    ...
    readChar();
      switch (charCodes[currentChar]) {
          ...
          case CHAR_PERIOD: token = makeToken(SB_LSEL, lineNo, colNo); readChar(); return token;
          default: return token;
        }
  case CHAR_RPAR: 
